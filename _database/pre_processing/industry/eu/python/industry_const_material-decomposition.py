...
# cdm_fert
cdm_fert = create_constant(df, ["fertilizer[t/t]"])
cdm_tra_infra = ConstantDataMatrix.create_from_constant(tmp, 1)
...
# 
# # get data
# filepath = os.path.join(current_file_directory, '../data/Literature/material_composition_literature_review.xlsx')
# df = pd.read_excel(filepath)
# 
# # melt
# indexes = ["material","material_sub"]
...
# products = ["LDV_ICE-gasoline[kg/num]","LDV_ICE-diesel[kg/num]","LDV_BEV[kg/num]","LDV_HEV[kg/num]"]
...
# df_temp1 = df_temp.loc[df_temp["material_sub"].isin(material_sub_pla),:]
# df_temp1 = df_temp1.groupby(["variable"], as_index=False)['value'].agg(sum)
# df_temp1["material"] = "plastics-total"
# df_temp = df_temp.loc[~df_temp["material_sub"].isin(material_sub_pla),:]
...
#                               products_fix=["cars-ICE[kg/num]","cars-ICE[kg/num]","cars-EV[kg/num]","cars-ICE[kg/num]"],
