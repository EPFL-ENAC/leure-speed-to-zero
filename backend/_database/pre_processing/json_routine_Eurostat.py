...
def eurostat_df_to_dm(df, filter, mapping_dim, unit):
    ...
    df = df.pivot(index=["Country","Years"], columns="var_long_name", values='value').reset_index()
    multi_index = pd.MultiIndex.from_product([df['Country'].unique(), df['Years'].unique()], names=['Country', 'Years'])
    ...
    if size_dim == 3:
        ...
        for c in ["Variables","Categories1"]:
            ...
            if len(names_original) == len(names_new):
                for original, new in zip(names_original, names_new): dm.rename_col(new, original, c)
            else:
...
def get_eurostat_jsonstat(dataset_code, *, compress=False, lang="en", extra_params=None, timeout=120):
    """
