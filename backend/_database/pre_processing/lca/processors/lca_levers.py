...
def get_material_footprint_df(current_file_directory, df_full):
    ...
    # save
    file_path = os.path.join(current_file_directory, '../data/intermediate_databases/materials.xlsx')
    if not os.path.exists(file_path): df_mat.to_excel(file_path)
...
def make_footprint_dm(df, first_year, years_start = None, years_end = None, years_gap = None, 
...
def make_other_dm(current_file_directory, df, pattern, years_ots, agg_prod_dict, years_gap = 1, deepen_n_cat = 1):
    ...
    # save
    file_path = os.path.join(current_file_directory, f'../data/intermediate_databases/{pattern}.xlsx')
    if not os.path.exists(file_path): df_sub.to_excel(file_path)

    # get ots data
    df_ots = df_sub.loc[df_sub['Scenario_year'].isin(['SSP5-Base_2025'])]

    # ots
    dm_ots = make_footprint_dm(df_ots, first_year=years_ots[-1], years_start=years_ots[0], years_end=years_ots[-2], years_gap=years_gap, 
                               agg_prod_dict = agg_prod_dict, deepen_n_cat = deepen_n_cat)
...
def run(years_ots):
    ...
    dm_mat = make_footprint_dm(df_ots_mat, years_ots[-1], years_ots[0], years_ots[-2], 1,
                               agg_prod_dict, agg_mat_dict)
    
    # get other footprint df
    df_other = get_other_footprint_df(df_full)
    
    # make dm for other footprints
    dm_ene_dem_elec = make_other_dm(current_file_directory, df_other, "energy-demand-elec", years_ots, agg_prod_dict)
    dm_ene_dem_ff = make_other_dm(current_file_directory, df_other, "energy-demand-ff", years_ots, agg_prod_dict)
    dm_eco = make_other_dm(current_file_directory, df_other, "ecological", years_ots, agg_prod_dict)
    dm_gwp = make_other_dm(current_file_directory, df_other, "gwp", years_ots, agg_prod_dict)
    dm_water = make_other_dm(current_file_directory, df_other, "water", years_ots, agg_prod_dict)
    dm_air = make_other_dm(current_file_directory, df_other, "air-pollutant", years_ots, agg_prod_dict)
    dm_heavy_metals = make_other_dm(current_file_directory, df_other, "heavy-metals", years_ots, agg_prod_dict)
    
    return dm_mat, dm_ene_dem_elec, dm_ene_dem_ff, dm_eco, dm_gwp, dm_water, dm_air, dm_heavy_metals
