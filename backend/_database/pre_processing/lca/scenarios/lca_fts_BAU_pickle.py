...
from _database.pre_processing.lca.processors.lca_levers import make_footprint_dm, make_aggregates_footprint

def load_data(current_file_directory, pattern):
    
    filepath = os.path.join(current_file_directory, f'../data/intermediate_databases/{pattern}.xlsx')
    df = pd.read_excel(filepath)
    
    return df

def make_dm_fts(df, years_fts, scenario, agg_prod_dict=None, agg_mat_dict=None, deepen_n_cat = 1):
    ...
    dm_fts_temp = make_footprint_dm(df_fts_temp, years_fts[-1], deepen_n_cat = deepen_n_cat)
...
def make_fts(current_file_directory, years_fts, DM_lca, dict_scenarios, variable, agg_prod_dict = None, agg_mat_dict = None):
    ...
    for level in dict_scenarios.keys():
        DM_lca["fts"]["footprint"][variable][level] = make_dm_fts(df_data, years_fts, dict_scenarios[level], 
...
def run(DM_lca, years_fts):
    ...
    # scenarios
    dict_scenarios = {1 : "SSP5-Base",  2: "SSP2-NPi", 3: "SSP2-NDC", 4: "SSP1-PkBudg1150"}
    
    # make fts
    make_fts(current_file_directory, years_fts, DM_lca, dict_scenarios, "materials", agg_prod_dict, agg_mat_dict)
    make_fts(current_file_directory, years_fts, DM_lca, dict_scenarios, 'energy-demand-elec', agg_prod_dict)
    make_fts(current_file_directory, years_fts, DM_lca, dict_scenarios, 'energy-demand-ff', agg_prod_dict)
    make_fts(current_file_directory, years_fts, DM_lca, dict_scenarios, 'ecological', agg_prod_dict)
    make_fts(current_file_directory, years_fts, DM_lca, dict_scenarios, 'gwp', agg_prod_dict)
    make_fts(current_file_directory, years_fts, DM_lca, dict_scenarios, 'water', agg_prod_dict)
    make_fts(current_file_directory, years_fts, DM_lca, dict_scenarios, 'air-pollutant', agg_prod_dict)
    make_fts(current_file_directory, years_fts, DM_lca, dict_scenarios, 'heavy-metals', agg_prod_dict)
    
    # # check
    # # Load existing DM_lca
    pickle_file = os.path.join(current_file_directory, '../../../data/datamatrix/lca.pickle')
