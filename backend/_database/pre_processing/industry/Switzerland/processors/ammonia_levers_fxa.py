...
from _database.pre_processing.industry.Switzerland.get_data_functions.data_ammonia_product_net_import import get_ammonia_trade_data
...
def make_ammonia_dms(dm, years_ots, years_fts):
    ...
    dm[...,"production","ammonia"] = dm[...,"production","ammonia"] * 1000 # get the right unit in tonnes
    ...
    dm_amm_prod_net_import[...] = 0.95 # making this correction to allow some local production of fertilizer (otherwise there is no
    # ammonia production)
...
def run(years_ots, years_fts):
    ...
    dm_amm_prod_net_import, dm_amm_mat_net_import, dm_amm_prod = make_ammonia_dms(dm_trade, years_ots, years_fts)
