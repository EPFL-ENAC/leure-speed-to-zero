...
def downscale_country_to_canton(
    ...
    # !FIXME: This method works for sure for categories where national additional capacity increases and cantonal available capacity increases
    # When national increases and cantonal decreases, it is a problem
    # When national decreases and cantonal decreases, it should be fine
    # When national decreases and cantonal increases it is a problem

    # country_prod = dm_prod_cap_cntr.col_labels['Country'][0]
    dm_prod_cap_cntr.drop("Categories1", ["Losses", "Net-import"])
    dm_cal_capacity.filter(
        {"Categories1": dm_prod_cap_cntr.col_labels["Categories1"]}, inplace=True
    )

    idx = dm_cal_capacity.idx
    dm_cal_capacity[:, idx[years_ots[-1]] :, "pow_existing-capacity", :] = (
        dm_cal_capacity[:, idx[years_ots[-1]], np.newaxis, "pow_existing-capacity", :]
    )

    # Compute the Delta between the current capacity installed and the max capacity for future years
    # Basically, compute the possible additional capacity installation
    dm_cal_capacity.operation(
        "pow_capacity-Pmax",
        "-",
        "pow_existing-capacity",
        out_col="pow_additional-capacity-available",
        unit="MW",
    )
    dm_additional_cap = dm_cal_capacity.filter(
        {"Variables": ["pow_additional-capacity-available"]}
    )

    # dm_additional_cap['Switzerland', :, 0, 'RoR'] = np.maximum(dm_additional_cap['Switzerland', :, 0, 'RoR'], 0.0005)
    dm_additional_cap.operation(
        country_dem, "/", country_prod, dim="Country", out_col="Vaud-share"
    )
    np.nan_to_num(x=dm_additional_cap.array, copy=False, nan=0)
    dm_additional_cap[
        :, 0 : idx[years_ots[-1]] + 1, "pow_additional-capacity-available", :
    ] = 0
    dm_additional_cap.filter({"Country": [country_dem + "-share"]}, inplace=True)
    dm_additional_cap.array = np.maximum(0, dm_additional_cap.array)
    # dm_additional_cap.rename_col('Vaud-share', 'Vaud', 'Country')

    # dm_additional_cap.append(dm_prod_cap_cntr.filter({'Variables': ['pow_capacity']})
    dm_additional_cap_cntr = dm_prod_cap_cntr.filter({"Variables": ["pow_capacity"]})
    dm_additional_cap_cntr.add(
        np.nan,
        dim="Variables",
    ...
    # Past existing-capacity = capacity
    dm_additional_cap_cntr[
        :, 0 : idx[years_ots[-1]] + 1, "pow_existing-capacity", :
    ] = dm_additional_cap_cntr[:, 0 : idx[years_ots[-1]] + 1, "pow_capacity", :]
    # Future "existing"-capacity = capacity(2023)
    dm_additional_cap_cntr[:, idx[years_ots[-1]] :, "pow_existing-capacity", :] = (
        dm_additional_cap_cntr[:, idx[years_ots[-1]], np.newaxis, "pow_capacity", :]
    )
    dm_additional_cap_cntr.operation(
        "pow_capacity",
        "-",
        "pow_existing-capacity",
        out_col="pow_additional-capacity-available",
    ...
    dm_additional_cap_cntr.filter(
        {"Variables": ["pow_additional-capacity-available"]}, inplace=True
    )

    # Merge
    dm_additional_cap.append(dm_additional_cap_cntr, dim="Country")
    dm_additional_cap.operation(
        country_prod, "*", country_dem + "-share", out_col=country_dem, dim="Country"
    )
    dm_additional_cap.filter({"Country": [country_dem]}, inplace=True)

    dm_cal_capacity.drop("Variables", ["pow_additional-capacity-available"])
    dm_cal_capacity.filter({"Country": [country_dem]}, inplace=True)
    dm_cal_capacity.append(dm_additional_cap, dim="Variables")
    dm_cal_capacity.operation(
        "pow_additional-capacity-available",
        "+",
        "pow_existing-capacity",
        out_col="pow_capacity",
        dim="Variables",
        unit="GW",
    )

    return dm_cal_capacity
...
def energyscope_pyomo(
    ...
    # downscale_country_to_canton(dm_prod_cap_cntr.copy(), dm_capacity.copy(), country_dem, country_prod, years_ots, share_of_pop)
