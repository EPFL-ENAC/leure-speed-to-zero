...
import model.emissions.interfaces as inter
import model.emissions.workflows as wkf
...
def emissions(years_setting, interface = Interface(), cntr_list = None, calibration = False):
    ...
    dm_transport = inter.get_interface(current_file_directory, interface, "transport", "emissions", cntr_list)
    dm_buildings = inter.get_interface(current_file_directory, interface, "buildings", "emissions", cntr_list)
    dm_industry = inter.get_interface(current_file_directory, interface, "industry", "emissions", cntr_list)
    dm_agriculture = inter.get_interface(current_file_directory, interface, "agriculture", "emissions", cntr_list)
    dm_ammonia = inter.get_interface(current_file_directory, interface, "ammonia", "emissions", cntr_list)
    DM_emi = {"transport" : dm_transport, "buildings" : dm_buildings, "industry" : dm_industry,
              "agriculture" : dm_agriculture, "ammonia" : dm_ammonia}
