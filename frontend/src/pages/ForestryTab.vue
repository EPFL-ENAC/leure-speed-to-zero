<template>
  <div>
    ...
    <div v-if="!modelResults" class="graph-placeholder q-pa-xl">
      ...
      <p>Run the model to see forestry data</p>
      <q-btn
        label="Run Model"
        color="primary"
        :loading="isLoading"
...
type ChartConfigs = typeof forestryConfig.charts;
type ChartId = keyof ChartConfigs;

const subtabs = forestryConfig.subtabs;
// No need for type assertion since TypeScript can infer this correctly
const charts = forestryConfig.charts;
...
if (!route.params.subtab && subtabs[0]?.route) {
  void router.replace({ name: 'forestry', params: { subtab: subtabs[0]?.route } });
}
...
watch(currentTab, async (newTab) => {
  if (newTab && newTab !== route.params.subtab) {
    try {
      await router.push({ name: 'forestry', params: { subtab: newTab } });
    } catch (error) {
...
const modelResults = computed(() => {
  const newData = leverStore.forestry;
