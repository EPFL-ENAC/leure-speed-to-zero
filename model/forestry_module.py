...
def forestry(lever_setting, years_setting, interface=Interface()):
    ...
    DM = read_data(forestry_data_file, lever_setting)
    #cntr_list = DM['key'].col_labels['Country']
    cntr_list = ['Switzerland']

    # If the input from Power/Energy are available in the interface, read them, else read from pickle
    if interface.has_link(from_sector='energy', to_sector='forestry'):
        dm_woodfuel = interface.get_link(from_sector='energy', to_sector='forestry')
    else:
        ...
        with open(lfs_interface_data_file, 'rb') as handle:
            dm_woodfuel = pickle.load(handle)
        dm_woodfuel.filter({'Country': cntr_list}, inplace=True)
