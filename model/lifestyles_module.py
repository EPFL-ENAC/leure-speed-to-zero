...
def read_data(data_file, lever_setting):
    ...
    # Appliances sub-matrix for the lifestyles dietary behaviour sub-flow
    ...
    return DM_food, DM_appliance, cdm_const
...
def food_workflow(DM_food,cdm_const):
    ...
    ay_diet_consumers=dm_diet_share.array[:, :, 0, :]*dm_diet_requirement.array[:,:,idx['lfs_healthy-gap'],np.newaxis]
    ...
    ay_total_fwaste = dm_diet_fwaste.array[:, :, 0, :] * dm_population.array[:, :, idx['lfs_population_total'],np.newaxis] \
...
def appliances_workflow(DM_appliance,cdm_const):
    ...
    ay_appliance_household = dm_household.array[:, :, idx_hsl['household-size'],np.newaxis] * dm_appliance_own.array[:, :,idx_appown['lfs_appliance-own'] ,:]
    dm_appliance_own.add(ay_appliance_household, dim='Variables', col_label='lfs_households-appliance-ownership', unit='#')

    # Phone use
    dm_appliance_use = DM_appliance['appliance-use']
    #dm_phone = dm_appliance_use.filter({'Categories1': ['phone']})
    idx_const = cdm_const.idx
    ...
    #ay_appliance_use_phone = dm_appliance_use.array[:,:,idx['lfs_appliance-use'],idx['phone']] * cdm_const.array[idx_const['cp_appliances_charging-time-share']]
    #dm_phone.add(ay_appliance_use_phone, dim='Variables', col_label='lfs_appliance-use_phone', unit='h')
...
def lifestyles(lever_setting, years_setting):
    ...
    DM_food, DM_appliance, cdm_const = read_data(lifestyles_data_file, lever_setting)
