...
def database_from_csv_to_datamatrix():
    ...
    # FIXME Weird data input
    # TODO Ensure the data is up to date with Paola's need for transport
    ...
    df = read_database_fxa('lifestyles_calibration-factors',filter_dict={'eucalc-name':'caf_lfs_food-wastes|caf_lfs_diet'})
    dm_caf_food = DataMatrix.create_from_df(df, num_cat=1)
    dict_fxa = {}
    dict_fxa['caf_food'] = dm_caf_food

    # Data - Constants
    cdm_const = ConstantDataMatrix.extract_constant('interactions_constants', pattern='cp_time_days-per-year.*',
                                                    num_cat=0)
...
def read_data(data_file, lever_setting):
    ...
    # Diet sub-matrix for the lifestyles dietry behaviour sub-flow
